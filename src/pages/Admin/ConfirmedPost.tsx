...
function PostManagerItem(props: any) {
	...
	const acceptPost = async () => {
		...
		if (result) {
			try {
				console.log("_id", props.post._id);
				const res = await postAPI.confirmPost(props.post._id, {
					status: 1,
				});
				console.log("ket qua duyet", res.data.result);
				props.refresh();
			...
			<td className="td__info__title">
				<div className="">
					<span className="span__badge badge__pill badge__warning">
						{renderTypeAccommodation(
							props.post.accommodation.typeAccommdation,
						)}
					</span>
					...
					<div className="div__container__info__typePost">
						<span>
							Loại tin: {renderTypePost(props.post.typePost)}
						</span>
